          ...
          >
            {items.map((d) => (
              <motion.a
                key={d.code}
                className="dest-card"
                href={`https://wa.me/18299952995?text=${encodeURIComponent(`Hola! Me interesa viajar a ${d.city} (${d.price})`)}`}
                target="_blank"
          ...
          >
            {(tab === 'daypass' ? dayPassPromos : tourPromos).map((p) => (
              <motion.a
                ...
                href={`https://wa.me/18299952995?text=${encodeURIComponent(`Hola! Me interesa el ${tab === 'daypass' ? 'Day Pass' : 'Tour'}: ${p.title} ${p.price ? `(${p.price})` : ''}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                variants={fadeInUp}
              >
